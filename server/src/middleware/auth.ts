...
function readSessionToken(c: Context<{ Bindings: Bindings; Variables: Variables }>): { token: string; salt: string } | null {
...
export async function getSessionUser(
  ...
  try {
    payload = (await decode({ token: found.token, secret: c.env.AUTH_SECRET, salt: found.salt })) as
      | JwtPayload
      | undefined
  } catch {
    return null
...
export async function requireApproved(
  ...
  const user = await getSessionUser(c)
  if (!user) return c.json({ error: 'unauthorized' }, 401)
  if (user.status !== 'approved') return c.json({ error: 'pending_approval' }, 403)
  c.set('user', user)
  await next()
}

...
export const adminHistoryRoute = new Hono<{ Bindings: Bindings; Variables: Variables }>()
...
adminHistoryRoute.get('/', async (c) => {
  ...
  if (scope && !SCOPES.includes(scope)) return c.json({ error: 'bad_scope' }, 400)

  // 정수만 통과시킨다 — NaN 이 SQL 로 흘러가면 LIMIT 의 의미가 흐려진다
  const limit = Math.min(Math.max(Math.trunc(Number(c.req.query('limit'))) || 50, 1), MAX_LIMIT)
  const page = Math.max(Math.trunc(Number(c.req.query('page'))) || 1, 1)

  const db: Sql = getDb(c.env)
  const rows = await db.query(LIST_SQL, [q || null, scope, limit, (page - 1) * limit])
  const total = (await db.query(COUNT_SQL, [q || null, scope])) as { n: number }[]

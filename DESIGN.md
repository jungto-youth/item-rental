...
- **계정 칩(헤더)**: 아바타 원(Action Blue 필 + 이니셜 흰 글자) + 이름 14px/600을 풀필 헤어라인 칩으로 묶어 네비 **밖** 오른쪽에 고정 — 메뉴 링크(muted 12px 텍스트)와 시각적 문법을 달리해 "메뉴"와 "내 계정"을 구분. 칩 클릭 = 마이페이지, 로그아웃은 별도 원형 아이콘 버튼(파괴적 동작이라 hover 시 danger). 560px 이하에선 이름을 숨기고 아바타만.

...
export class XCalendar extends LitElement {
  ...
  private get maxDate(): string {
    return (
      this.days[this.days.length - 1]?.date ?? addDays(fmt(new Date()), 89)
    );
  }
  ...
  private disabledReason(d: string, busy: Set<string>): Reason {
    ...
    if (this.startDate) {
      ...
      for (let s = this.startDate; s < d; s = addDays(s, 1))
        if (busy.has(s)) return "span";
      return "";
    }
  ...
  private roveTo(target: string) {
    ...
    void this.updateComplete.then(() => {
      this.renderRoot
        .querySelector<HTMLButtonElement>(`[data-date="${t}"]`)
        ?.focus();
    });
  }
  ...
  private onDayKey(e: KeyboardEvent, date: string) {
    ...
    switch (e.key) {
      ...
      case "ArrowUp":
        ...
        break;
      case "Home":
        target = fmt(new Date(this.viewY, this.viewM - 1, 1));
        break; // 그 달의 첫 선택 가능일로 클램프됨
      case "End":
        target = fmt(new Date(this.viewY, this.viewM, 0));
        break;
      case "PageUp":
        this.move(-1, true);
        e.preventDefault();
  ...
  protected updated() {
    if (
      this.cursor &&
      (this.cursor < this.boundStart || this.cursor > this.maxDate)
    ) {
  ...
  private cells(): (string | null)[] {
    ...
    for (let i = 1; i <= last; i++)
      out.push(fmt(new Date(this.viewY, this.viewM - 1, i)));
    return out;
  }
  ...
  static styles = [
    ...
    css`
      ...
      .day:not([aria-disabled="true"]):active {
        transform: scale(0.9);
      }

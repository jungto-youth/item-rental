...
export class AvailabilityStrip extends LitElement {
  ...
  static styles = css`
    ...
    .bar i { flex: 1; background: var(--color-success); opacity: 0.45; }
    .bar i.partial { background: var(--color-warning); opacity: 0.55; }
    .bar i.busy { background: var(--color-danger); opacity: 0.9; }
    .legend {
      display: flex;
      gap: var(--space-3);
    ...
    return html`
      ...
      ${this.busyRanges().length > 0
        ? html`
            <ul>
              ${this.busyRanges().map(
                (r) => html`<li>· ${fmt(r.start)}${r.start !== r.end ? ` ~ ${fmt(r.end)}` : ''} 전량 예약</li>`,
              )}
            </ul>
          `
        : ''}

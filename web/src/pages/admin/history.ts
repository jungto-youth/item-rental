...
export class PageAdminHistory extends LitElement {
  ...
  static styles = css`
    ...
    .count { color: var(--color-muted); font-size: var(--text-caption); }
    .rows { display: grid; }
    .row {
    ...
    .more[disabled] { color: var(--color-muted); cursor: default; }
  `
  ...
  private async reload(append = false) {
    ...
    try {
      const params = new URLSearchParams({ page: String(this.page), limit: String(LIMIT) })
  ...
  render() {
    return html`
      <h1>대여 이력</h1>
      <p class="count">
        2025 청년페스타 시트 기록이에요 — 지난 대여를 찾아볼 때 쓰는 참고 자료이고,
        예약 처리에는 관여하지 않아요.
      </p>
      <div class="bar">
        <input
        ...
        <select .value=${this.scope} aria-label="구분" @change=${this.onScope}>
          ...
          <option value="청년물품" ?selected=${this.scope === '청년물품'}>청년물품</option>
          <option value="회관물품" ?selected=${this.scope === '회관물품'}>회관물품</option>
        </select>
        <span class="count">${this.loading ? '' : `총 ${this.total}건`}</span>
      </div>
  ...
  private renderRows() {
    return html`
      ...
      ${this.rows.length < this.total
        ? html`<button class="more" ?disabled=${this.loadingMore} @click=${() => this.more()}>
  ...
  private renderRow(r: RentalHistoryRow) {
    ...
    const who = [r.item_scope, r.member_name, r.org].filter(Boolean).join(' · ')
    const when = [
      r.requested_on ? `신청 ${r.requested_on}` : '신청일 미상',
      period(r.start_at, r.end_at) ? `사용 ${period(r.start_at, r.end_at)}` : '',
      r.use_location ?? '',
      r.qty ? `수량 ${r.qty}` : '',
    ]
      .filter(Boolean)
    ...
    return html`
      <div class="row">
        <span class="head">
          ...
          ${r.item_id
            ? html`<button class="link" @click=${() => navigate(`/items/${r.item_id}`)}>
                물품 보기
              </button>`
            : ''}
        </span>
